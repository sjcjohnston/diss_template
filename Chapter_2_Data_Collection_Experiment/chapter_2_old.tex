...
The speech vibrations of a person's own voice will propagate throughout the head and body.% (cf. Fig. \ref{fig:bekesyBodyTransfer}).  
Of interest for the present study, these waves will pass through the tissue in the head, and enter into the ear canal, where they will be recorded. % This brief section will discuss some of the known properties 
...
There have been many investigations of the resonating characteristics and amplitude response of the ear canal.  One such project was performed by \cite{stinson:89}, which studied fifteen human ear canals.  Their aim was to produce a model which could replicate the effect that the ear canal has on acoustics.
...
Between each class that was used - voiceless sounds (/s/, /t/, /k/, and /tj/),  nasals (/m/ and /n/), and vowels (/i/, /e/, /a/, /o/) - a moderately similar frequency response is seen, yet there are some distinctions to note. %(see Fig. \ref{BCrelACall}).
...
\cite{reinfeldt:10}'s findings are backed by \cite{bekesy:60}, who also noted %diagrammed 
...
% \end{wrapfigure}
%
The study conducted its own tests %(seen in Fig. \ref{fig:hansenAverageOEb}), 
...
It was not possible to satisfy all `requirements' for all speakers, but the first three conditions were met for each of the twenty participants used in the present study.  Survey answers from all participants were used.
