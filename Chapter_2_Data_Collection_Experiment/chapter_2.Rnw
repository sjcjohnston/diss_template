...
<<ANOVA_Data_collection, echo=FALSE, results='asis'>>=
...
female=mydata[mydata$speaker_gender=="f",]
female$speaker_gender=as.factor(female$speaker_gender)
male = mydata[mydata$speaker_gender=="m",]
...
by_subj_ez_anova = ezANOVA(
	...
	, within = .(speaker_gender,processing_level) #(independent variables/factors)
	)
...
by_item_ez_anova = ezANOVA(
	...
	, dv = xcorr
	, wid = sentence
	, within = .(speaker_gender,processing_level)
	)
...
<<ANOVA_data_collection_results, echo=FALSE, results='asis'>>=
