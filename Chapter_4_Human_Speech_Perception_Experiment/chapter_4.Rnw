...
boxplot(wer~noise_type,col=c("white","lightgray"),mydata, las=2, names=c("No Noise","Bus","Cafe","Pedestrian","Street","Factory"),ylab="WER")
@
...
boxplot(wer~mic_location,col=c("white","lightgray"),mydata, names=c("Ear-Recorded","Mouth-Recorded"),ylab="WER")
@
...
#par(mar=c(6,5,2,2))
boxplot(wer~droplevels(noise_type),col=c("white","lightgray"),noiseless_data, names=c("Bus","Cafe","Pedestrian","Street","Factory"),ylab="WER",las=2)
@
...
plot(data1$sent_num, data1$wer, pch = 16, cex = 1.3, col = "black", main = "Linear Regression of performance over time", xlab = "Stimulus Presentation Order", ylab = "WER")
...
Four native speakers of English with self-reported normal hearing participated in this study.  The design and procedure of this experiment was exactly the same as the initial perception experiment, save the alteration in modifications performed on the ear-recorded stimuli, described above.
...
Since there were only 5 participants in this follow-up study, no actual statistics could be run to test for significance.  The results here are to be used in a basic comparison of possible directions these altered methods may be able to take future research.

The boxplot in Figure \ref{fig:F0_noiseXmic_boxplot} demonstrates the interaction of most interest, that of noise-type and microphone location, as identified by the ANOVAs performed on the primary experiment above.  There was no change to any of the mouth-recorded speech signals, so there ought not be any change ot the performance of these signals outside of human listener variation that is to be expected.

\begin{figure}{t}%{L}{\textwidth}
...
It is interesting to note that the difference between the clean ear speech and the ``noisy'' ear speech has essentially disappeared. The speech added to the clean ear-recorded speech would have been from the simultaneous clean mouth-recorded speech, and so there is still no noise present in these ear-recorded signals.  From this plot, it appears that - if this follow up study were to be extended - there would be a dramatic difference between the noisy speech recorded at the mouth, and the speech recorded at the ear.
Further discussion concerning the comparison of these results with those of the primary study and the ``perceptual learning'' study can be found below in Section \ref{h4:glob_discussion}.
...
par(mar=c(6,5,2,2))
boxplot(wer~noise_type*droplevels(mic_location)*style,col=c("white","lightgray"),alldata1, las=2, names=c("No Noise.R","Bus.R","Cafe.R","Pedestrian.R","Street.R","Factory.R","No Noise.F","Bus.F","Cafe.F","Pedestrian.F","Street.F","Factory.F","No Noise.T","Bus.T","Cafe.T","Pedestrian.T","Street.T","Factory.T"),ylab="WER",las=2)
@
...
par(mar=c(6,5,2,2))
boxplot(wer~noise_type*droplevels(mic_location)*style,col=c("white","lightgray"),alldata2, las=2, names=c("No Noise.R","Bus.R","Cafe.R","Pedestrian.R","Street.R","Factory.R","No Noise.F","Bus.F","Cafe.F","Pedestrian.F","Street.F","Factory.F","No Noise.T","Bus.T","Cafe.T","Pedestrian.T","Street.T","Factory.T"),ylab="WER",las=2)
@
