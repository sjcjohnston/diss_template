...
Of interest for the present study, these waves \DIFdelbegin \DIFdel{will }\DIFdelend pass through the tissue in the head, and enter into the ear canal, where they \DIFdelbegin \DIFdel{will }\DIFdelend \DIFaddbegin \DIFadd{can }\DIFaddend be recorded. % This brief section will discuss some of the known properties 
...
\DIFdelbegin \DIFdel{is 
  ...
  \item  is not as easily manipulated as a simple mechanically-created vibration \DIFaddbegin \DIFadd{(\mbox{%DIFAUXCMD
\cite{}
}%DIFAUXCMD
)}\DIFaddend ,
  \item  contains far more frequency components than a simple mechanically-created vibration \DIFaddbegin \DIFadd{(\mbox{%DIFAUXCMD
\cite{}
}%DIFAUXCMD
)}\DIFaddend , and 
...
One challenge in producing such a model is the considerable variability in the shape of the canal - both between subjects as well as between the right and left ear canal of a single subject (\cite{stinson:89}).  These differences are apparent in curvature, length, volume, and cross-sectional diameter throughout the ear canal.  \cite{stinson:89} created silicon ear molds for each of the ear canals, which were used to generate three different computational models: one following the contours and dimensions of their ear molds exactly, another following the dimensions of the ear mold, but straightening contours and curvatures as if along a central axis, and the third as if the ear canal were a uniform tube with the same length and volume of the ear canal molds and previous models\DIFdelbegin \DIFdel{(see Fig. \ref{fig:eac_modelling}).}\DIFdelend \DIFaddbegin \DIFadd{.%DIF >  (see Fig. \ref{fig:eac_modelling}).  
...
\cite{porschmann:00} \DIFdelbegin \DIFdel{'s study }\DIFdelend is generally looking at the \textit{self-perception} of one's own voice, but in order to accomplish this devotes effort to looking at the bone conduction pathway separately.  A general 0.9 kHz resonance (with subsequent harmonic resonances) was found in the collected bone-conduction speech, generally present between 0.7 and 1.2 kHz.  This correlates with the 0.8-1.2 kHz range for the first resonance that others (cf. \cite{hakansson:94}) have observed in mechanical-stimulated bone conduction studies. This would mean, in terms of speech, that one would expect to find higher relative amplitudes near the first formant.
...
\cite{reinfeldt:10}, use microphones to record the actual sound pressure level (SPL) of both air- and body-conducted speech. Furthermore, \cite{reinfeldt:10} used a more diverse set of \DIFdelbegin \DIFdel{phones}\DIFdelend \DIFaddbegin \DIFadd{phonemes}\DIFaddend .  A resonance was found in generally the same frequency region for /s/ (and other \DIFdelbegin \DIFdel{phones}\DIFdelend \DIFaddbegin \DIFadd{phonemes}\DIFaddend ) as that found by \cite{porschmann:00} (0.7 - 1.2 kHz) and \cite{hakansson:94} (0.8 - 1.2 kHz). %, they discovered some distinct differences, which can be seen in Fig. \ref{BCrelAC}. 
Between each class that was used - voiceless sounds (/s/, /t/, /k/, and /tj/),  nasals (/m/ and /n/), and vowels (/i/, /e/, /\DIFdelbegin \DIFdel{a}\DIFdelend \DIFaddbegin \DIFadd{\textipa{A}}\DIFaddend /, /o/) - a moderately similar frequency response is seen, yet there are some distinctions to note. %(see Fig. \ref{BCrelACall}).
...
which also supports this hypothesis.  There was much inter-speaker variability in the previous studies, but \DIFdelbegin \DIFdel{the bone-conducted vowels appeared to have }\DIFdelend \DIFaddbegin \DIFadd{one common characteristic is that the higher vowels had }\DIFaddend the least relative reduction in amplitude\DIFdelbegin \DIFdel{are the higher vowels}\DIFdelend .  Since the functions of other high sonorants (eg. /u/) are not given, we cannot be certain if this is a \DIFdelbegin \DIFdel{phone-specific }\DIFdelend \DIFaddbegin \DIFadd{phoneme-specific }\DIFaddend difference, or if it can be generalized to other sounds with a [high] articulation in which the tongue is close to the roof of the mouth.  This latter generalization would make sense, as the oral cavity is more `closed', trapping energy inside the cavity resulting in more reverberations passing through the head and into the ear canal.
%
% \begin{wrapfigure}{l!}{0.5\textwidth}
% % \begin{figure}
...
Some have posited that variability could critically stem from the placement of the jaw bone during speech next to the external auditory meatus, and as that changes as the jaw moves up and down for `higher' or `lower' phones (eg. /i/ vs /\DIFdelbegin \DIFdel{a}\DIFdelend \DIFaddbegin \DIFadd{\textipa{A}}\DIFaddend /). The placement of the jaw bone, as it moves changes the impedance characteristics of the vibration of the mandible against the temporal bone that houses the meatus (\cite{bekesy:60}).  \cite{allen:60} studied the \DIFdelbegin \DIFdel{OE }\DIFdelend \DIFaddbegin \DIFadd{occlusion effect }\DIFaddend on participants with a unilateral resection of the mandible (one side of the jaw has been removed), and found essentially no distinction between the \DIFdelbegin \DIFdel{OE }\DIFdelend \DIFaddbegin \DIFadd{occlusion effect }\DIFaddend in either ear (ie. with a mandibular joint adjacent to the cartilage of the ear canal or without).
...
Sentences were identified and extracted using Praat.  A number of subjects (more than 20) were recorded, but in an attempt to meet a number of conditions, only 20 participants used for analysis in this study \DIFdelbegin \DIFdel{.  These conditions included:
...
\begin{figure}[h!]
\centering
\DIFdelbeginFL %DIFDELCMD < \begin{subfigure}{.475\textwidth}
%DIFDELCMD <   %%%
...
\end{subfigure}%
\DIFdelbeginFL %DIFDELCMD < \hfill
%DIFDELCMD < \begin{subfigure}{.475\textwidth}
%DIFDELCMD <   %%%
\DIFdelendFL %DIF >  \hfill
\DIFaddbeginFL 
...
\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \begin{subfigure}{0.475\textwidth}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \begin{subfigure}{\textwidth} %DIF > 0.475
  \DIFaddendFL \centering
...
A \DIFaddbegin \DIFadd{Long-Term Average Spectrum (LTAS) was calculated for both ear-recorded speech and mouth-recorded speech, averaging over all male speakers and separately over all female speakers.  Figure \ref{} shows the LTAS for the male speakers, along with the relative amplitude difference between the ear-recorded and mouth-recorded speech.  Figure \ref{} displays the LTAS and difference function for all female speakers.  Both have the same basic characteristics - while the mouth-recorded speech has a relatively level LTAS across the frequency spectrum, the ear-recorded speech is heavily low-pass filtered.  The ear-recorded signal dominates in the low-frequencies, but is overtaken by the mouth-recorded signal at only about 500 Hz.  
}
...
\begin{subfigure}{\textwidth} %DIF > 0.475
  \centering
  \includegraphics[width=1\linewidth]{figure/female_LTAS.png}
...
\begin{subfigure}{\textwidth}
  \centering
  \includegraphics[width=1\linewidth]{figure/female_rel_dB.png}
...
\begin{subfigure}{\textwidth} %DIF > 0.475
  \centering
  \includegraphics[width=1\linewidth]{figure/male_LTAS.png}
...
\begin{subfigure}{\textwidth}
  \centering
  \includegraphics[width=1\linewidth]{figure/male_rel_dB.png}
...
), the recorded signals appear to be quite low-pass filtered.  The drop in amplitude for these signals is immediate as the frequency increases, whereas \mbox{%DIFAUXCMD
\cite{hansen:97b}
}%DIFAUXCMD
(also \mbox{%DIFAUXCMD
\cite{wimmer:86,thorup:96,may:92}
}%DIFAUXCMD
, among others) predicted higher amplitude until approximately 1 kHz.  The sharp decline in amplitude levels briefly slightly above 1 kHz, and an argument could be made that this is the `resonance' of the ear canal that was identified between 0.7 and 1.2 kHz by \mbox{%DIFAUXCMD
\cite{porschmann:00}
}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\cite{hakansson:94}
}%DIFAUXCMD
, among others, but this is only speculation.  %DIF > One possibility is the depth of the plug.  \cite{dean:01} and \cite{stenfelt:07} both found that the occlusion effect intensified toward the lower frequencies as the plug depth increased, and the range of amplified frequencies became lower and lower.
}
...
\DIFadd{A }\DIFaddend cross-correlation similarity metric used to compare signals was run on each ear-recorded/mouth-recorded pair for each participant speaker (excluding those which contained noise in the background).  \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Recall that these signals were recorded simultaneously, and were also extracted using the same Praat TextGrid boundaries.  Additionally, the signals were time-aligned using a variation of the Matlab code found in Appendix Section \ref{app:time-align}.  This normally resulted in an additional shift of $<$10 samples.  For the cross-correlation calculation, the }\DIFaddend maximum value was obtained for each ear-recorded/mouth-recorded pair, was normalized, and then all values were averaged across speakers.  This is given in Equation \ref{norm_max_xcorr}:
%
...
%
where $K$ is the number of participants, $U$ is the number of utterances, \DIFdelbegin \DIFdel{$S^e$ }\DIFdelend \DIFaddbegin \DIFadd{$s^e$ }\DIFaddend refers to the ear-recorded signal \DIFdelbegin \DIFdel{, $S^m$ }\DIFdelend \DIFaddbegin \DIFadd{in the time domain, $s^m$ }\DIFaddend refers to the mouth-recorded signal \DIFaddbegin \DIFadd{in the time domain}\DIFaddend , and $xcorr$ is the cross-correlation function. If \DIFdelbegin \DIFdel{$S^e_{uk}$ and $S^m_{uk}$ }\DIFdelend \DIFaddbegin \DIFadd{$s^e_{uk}$ and $s^m_{uk}$ }\DIFaddend are identical signals, the fractional portion of Equation \ref{norm_max_xcorr} will return a `1'.  The average normalized maximum cross-correlation value for ear-recorded speech compared with mouth-recorded speech is 0.358.
...
\end{subfigure}
\caption{Both \ref{spctgrmNarrowMouthNoise_35} and \ref{spctgrmNarrowEarNoise_35} were of the sentence ``A cramp is no small danger on a swim'', spoken by a female participant, recorded simultaneously.  \DIFaddbeginFL \DIFaddFL{The high-energy bands at 2 kHz and in the frequency spectrum are characteristic of the bus background noise (cf. Fig. \ref{fig:bkgrnd-noises}).}\DIFaddendFL }
...
\end{subfigure}
\caption{Narrow band spectrograms (\ref{spctgrmEarNarrow20kHz} and \ref{spctgrmNarrowEarNoisePremp_35}) of ear-recorded speech from 0-20 kHz to look for possible speech information in higher frequencies, of the sentence ``A cramp is no small danger on a swim'' spoken by a female participant. Note, the noise only extends to \DIFdelbeginFL \DIFdelFL{8kHz}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{8 kHz}\DIFaddendFL . Underneath (\ref{spctrmEar20kHz} and \ref{spctrmEar20kPreemp}) are two spectra from the same speaker and same sentence, normal and pre-emphasized, respectively.}
\end{figure}
...
\textbf{\DIFaddFL{Cross-Correlation of Ear- and Mouth-Recorded Speech}}
\DIFaddendFL 
...
%DIFDELCMD < %%%
\DIFdelend %DIF >  \subsection{Survey Responses}
...
\DIFaddbegin \DIFadd{and answers. %DIF > , which are located in Table \ref{tab:survey-results}.
%DIF >  \begin{table}
%DIF >  \centering
...
).  Participants also described that }\DIFaddend they would be \DIFdelbegin \DIFdel{willing to use the device in a real-life application (Q3).  The mode is rather high (5) given the mean (3.51), which could be due in part to participant response bias (ie. desiring to provide an `acceptable' response)}\DIFdelend \DIFaddbegin \DIFadd{more likely to use such a recording equipment if the noise-reduction ear muffs were not required}\DIFaddend .
